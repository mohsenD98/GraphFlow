...
Rectangle {
  ...
  function canLink(fromAttribute, toAttribute) {
    return (fromAttribute && toAttribute && fromAttribute.output.visible && toAttribute.input.visible && toAttribute.boundInputs.length <= 2 && fromAttribute.node != toAttribute.node);
  }
}
